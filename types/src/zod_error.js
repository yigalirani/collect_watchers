import { red, reset, yellow } from "@yigal/base_types";
...
function format_message(input) {
    ...
    output = output.replace(/(expected)(.*?)(received)/i, (_, exp, mid, rec) => exp + yellow + mid + reset + rec);
    // Highlight text after "received:"
    output = output.replace(/(received:)(.*)$/i, (_, rec, after) => rec + red + after + reset);
...
export function consolidate_errors(log) {
    const ans = [];
...
export function format_zod_error(ex) {
    //console.log('orig',ex)
    ...
    function f(ar, acum_path, level) {
        ...
        log.push({ path: acum_path.join('/'), message: message });
    }
    f(top[0], [], 0);
